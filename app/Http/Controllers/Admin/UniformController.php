...
use App\Models\UniformDrivers;
use App\Models\UniformDriversItems;
use Carbon\Carbon;
...
class UniformController extends Controller
{
    public function index()
    {
        ...
        $uniformDrivers = get_cols_where_p(new UniformDrivers(), array("*"), array("com_code" => $com_code), 'id', 'desc', PC);
        return view("admin.Uniform.index", ['data' => $data,'uniformDrivers'=>$uniformDrivers]);
    }
    ...
    public function edit($driver_id){
        ...
        $deposits = get_cols_where(new Uniform(), array("*"), array('com_code' => $com_code));
        if (empty($data)) {
        return redirect()->route('uniform_Receive.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة']);
        }
        return view('admin.Uniform.edit', ['data' => $data ,'deposits' => $deposits]);
    ...
    public function storeUniform(Request $request)
    {
        ...
        [
            'inputs.*.name'=>'الاسم مطلوب',
                ...
                //  $DataToInsert['name'] = $request->note;
                 ...
                 $note= insert(new UniformDrivers(), $DataToInsert);
            if (!$note) {
        ...
        foreach ($request->inputs as $key => $value) {
            ...
            UniformDriversItems::create($value);
        }
        // return back()->with('success','تم تسليم الامانات');
        return redirect()->route('uniform_Receive.index')->with(['success' => 'Uniform have been successfully delivered.']);
    }
    ...
    //   $data = get_cols_where_p(new UniformDrivers(), array("*"), array("com_code" => $com_code), 'id', 'desc', PC);
  
    //   $dailyEmployeesReportTasks = get_cols_where(new UniformDriversItems(), array("*"), array("com_code" => $com_code),'id','ASC');

    //   $admins = get_cols_where(new Admin(), array("*"), array("com_code" => 1));
 
    //     return view('admin.Uniform.show_unform_drivers', ['data' => $data,'dailyEmployeesReportTasks'=>$dailyEmployeesReportTasks,'admins'=>$admins]);
    ...
    public function show_drivers_deposit()
    {
      ...
      $driversUniform = get_cols_where_p(new UniformDrivers(), array("*"), array("com_code" => $com_code,"report_status"=>0), 'id', 'desc', PC);
  
      $driversUniformItems = get_cols_where(new UniformDriversItems(), array("*"), array("com_code" => $com_code),'id','ASC');

      $admins = get_cols_where(new Admin(), array("*"), array("com_code" => 1));
 
        return view('admin.Uniform.show_uniform_drivers2', ['data' => $data,'driversUniform'=>$driversUniform,'driversUniformItems'=>$driversUniformItems,'admins'=>$admins]);
   ...
   public function ajax_update_report_status(Request $request)
   {
       if ($request->ajax()) {
           try {
             ...
             $data = get_cols_where_row(new UniformDrivers(), array("*"), array('com_code' => $com_code, 'id' => $request->report_id_value));
               if (!empty($data)) {
                   echo json_encode("found");
               }
               DB::beginTransaction();
               update(new UniformDrivers(),$data_to_update, array("com_code" => $com_code,'id' => $request->report_id_value));
               DB::commit();
...
//         $search_by_supervisor = $request->search_by_supervisor;
...
//         $data = UniformDrivers::select("*")->whereIn('driver_id',$employee_id)->where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->orderby('id', 'DESC')->paginate(PC);
//         $dailyEmployeesReportTasks = get_cols_where(new UniformDriversItems(), array("*"), array("com_code" => 1));
//         $admins = get_cols_where(new Admin(), array("*"), array("com_code" => 1));

     

//         return view('admin.Uniform.ajax_daily_report_search', ['data' => $data,'dailyEmployeesReportTasks'=>$dailyEmployeesReportTasks,'admins'=>$admins,'today'=>$report_date_search]);
//         }
// }
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $uniformDrivers = get_cols_where_p(new UniformDrivers(), array("*"), array("com_code" => $com_code), 'id', 'desc', PC);
            return view("admin.Uniform.ajax_search", ['data' => $data,'uniformDrivers'=>$uniformDrivers]);



            // $driversdeposit = get_cols_where_p(new UniformDrivers(), array("*"), array("com_code" => $com_code), 'id', 'desc', PC);
            // return view('admin.Uniform.ajax_search', ['data' => $data,'driversdeposit'=>$driversdeposit]);
        }
    }
    /////////////////////////////////////////////////////////////////////////////
     /////////////////////////////////////////////////////////////////////////////

     public function ajax_show_drivers_uniform_search(Request $request)
     {
         if ($request->ajax()) {
             $searchbyradio = $request->searchbyradio;
             $search_by_text = $request->search_by_text;
             ...
             $admins = get_cols_where(new Admin(), array("*"), array("com_code" => 1));
               ...
               return view('admin.Uniform.ajax_daily_report_search', ['data' => $data,'driversUniform'=>$driversUniform,'driversUniformItems'=>$driversUniformItems,'admins'=>$admins]);
  
            // $driversdeposit = get_cols_where_p(new UniformDrivers(), array("*"), array("com_code" => $com_code,"report_status"=>0), 'id', 'desc', PC);


            // $driversDepositItems = get_cols_where(new UniformDriversItems(), array("*"), array("com_code" => $com_code),'id','ASC');

            // $admins = get_cols_where(new Admin(), array("*"), array("com_code" => 1));
       
            //   return view('admin.Uniform.ajax_daily_report_search', ['data' => $data,'driversdeposit'=>$driversdeposit,'driversDepositItems'=>$driversDepositItems,'admins'=>$admins]);
