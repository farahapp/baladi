...
<a href="{{ route('Maintenance.index') }}">   السائقين   </a>
@endsection
...
<div class="col-12">
   <div class="card">
      ...
      <div class="row" style="padding: 5px;">
         ...
         <div class="col-md-3">
            <div class="form-group">
               <label> البحث عن طريق حالة الإقامة </label>
               <select name="residency_process_status_search" id="residency_process_status_search" class="form-control select2 ">
                  <option  value="all">البحث بالكل </option>
                  @if ($other['residency_process_status'] && !@empty($other['residency_process_status']))
                  @foreach ($other['residency_process_status'] as $residency_status )
                  <option @if(old('residency_process_status_search')==$residency_status->id) selected="selected" @endif value="{{ $residency_status->id }}"> {{$residency_status->name}} </option>
                  @endforeach
                  @endif
               </select>
            </div>
         ...
         <div class="col-md-3">
            <div class="form-group">
               <label> البحث عن طريق حالة البنك </label>
               <select name="driver_bank_process_search" id="driver_bank_process_search" class="form-control select2 ">
                  <option  value="all">البحث بالكل </option>
                  @if ($other['driver_bank_process'] && !@empty($other['driver_bank_process']))
                  @foreach ($other['driver_bank_process'] as $bank_process )
                  <option @if(old('driver_bank_process_search')==$bank_process->id) selected="selected" @endif value="{{ $bank_process->id }}"> {{$bank_process->name}} </option>
                  @endforeach
                  @endif
               </select>
            </div>
...
<script src="{{ secure_asset('assets/admin/js/government_process.js') }}"></script>
<script>
