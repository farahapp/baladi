...
<div class="col-12">
   <div class="card">
      ...
      <div class="card-body" id="driving_school_status_searchDiv"  style="background-size: cover; background-image: url('{{ secure_asset('assets/admin/imgs/doha.webp') }}')">
        ...
        border-radius: .8rem;overflow: hidden;" class="custom_img"  alt="Avatar"  id="showImageView"  src="{{ secure_asset('assets/admin/uploads').'/'.$data['driver_photo'] }}" alt="الصورة الشخصية للسائق" ><br/>
                  <button type="button" class="btn btn-sm  showImageButton"   value="{{ $data['driver_photo'] }}" style="width:150px;color:white;background-color: #FBA82E" value="initial_contract_image">View image</button>
                  @endif
            </div>
            <label style="box-shadow: 0 .4rem .8rem #0005;
        border-radius: .8rem;overflow: hidden;color:#EF4C2B">{{ old('driver_name',$data['driver_name']) }}</label>
         </div>   
         
         <form action="{{ route('SecurityGuard.store') }}" method="post" enctype="multipart/form-data">
            @csrf
         ...
         <button style="display: none;color:white;background-color: #FBA82E"  type="submit" id="confirm_deposit"name="confirm_deposit" class="btn  col-md-3">Confirm deposit withdrawal</button>
         </div>
      </section>
   </main>
      </form>
         ...
         <form  method="post">
            {{-- <form  method="post"> --}}
               ...
               deposit_id="{{ $raw['id'] }}"
               style="margin-top:5px;color:white;background-color: #EF4C2B"  value="Add Deposit" />
             </div>
...
<div class="modal fade  "   id="show_imageModal">
   <div class="modal-dialog modal-xl"  >
      <div class="modal-content"  style="color:white;background-color: #FBA82E">
...
<script>
   ...
   $('#table').append(`
            <tr>
               ...
               <td data-lable="">
                  <input readonly type="text" name="inputs[`+i+`][size]" value=" `+deposit_size+`"  placeholder="size" class="form-control">
               </td>
